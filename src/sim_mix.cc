...
int main(int argc,char **argv) {
    ...
    if(nobjs>0) {
        ...
        // Small mixing example objects = 100
        if(nobjs==100) {
            // Top levels: heavier solids
            for(int i=-5;i<5;i++) {
                for(int j=0;j<5;j++) {
                    rho_s=1.25;
                    grav=convert_set_force(rho_f,rho_s,C_g);
                    int sx=nx/nx,sy=ny/ny;
                    switch(obj_type) {
                        case 0: os=new obj_circle(cx*nx+(i+0.5)*(2*cr*fmin(nx,ny)+5.)*sx,cy*ny+(j+0.5)*2*(cr*fmin(nx,ny)+2.5)*sy,cr*fmin(nx,ny),grav);
                                objtext="circle";
                                break;
                    }
                    objs.push_back(os);
        ...
        // Big mixing example objects = 1000
        else if(nobjs==500) {
            // Top levels: heavier solids
            for(int i=-11;i<11;i++) {
                for(int j=0;j<11;j++) {
                    ...
                    int sx=nx/nx,sy=ny/ny;
                    switch(obj_type) {
                        case 7: os=new obj_ellipse(cx*nx+(i+0.5)*(2.5*cr*nx)*sx,cy*ny+(j+0.5)*2*(1.25*cr*ny)*sy,cr*fmin(nx,ny),0.25,1.,grav);
                                objtext="ellipse";
                                break;
                    }
                    objs.push_back(os);
            ...
            for(int i=-22;i<22;i++) {
                for(int j=-6;j<0;j++) {
                    ...
                    int sx=nx/nx,sy=ny/ny;
                    switch(obj_type) {
                        case 7: os=new obj_ellipse(cx*nx+(i+0.5)*(1.25*cr*nx)*sx,cy*ny+(j+0.5)*2*(2.25*cr*ny)*sy,cr*fmin(nx,ny),1.,0.25,grav);
                                objtext="ellipse";
                                break;
                    }
                    objs.push_back(os);
