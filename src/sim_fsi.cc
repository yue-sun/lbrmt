...
int main(int argc,char **argv) {
    ...
    if(argc!=2&&(argc!=3||strcmp(argv[1],"-m")!=0)) {
        fputs("Syntax: ./sim_lbrmt_2d_solid [-m] <input_file>\n\n"
    ...
    if(nobjs>0) {
        ...
        double C_g=dx/dt/dt;          // Conversion factor of force density
        double grav=convert_set_force(rho_f,rho_s,C_g);
        ...
        std::string objtext;
        int sqnobjs=int(ceil(sqrt(nobjs)));
        for(int j=0;j<sqnobjs;j++) {
            for(int i=0;i<sqnobjs;i++) {
                switch(obj_type) {
                    case 0: os=new obj_circle(cx*nx+(i-sqnobjs/2)*2.5*cr*nx,cy*ny+(j-sqnobjs/2)*2.5*cr*ny,cr*nx,grav);
                            objtext="circle";
                            break;
                    case 1: os=new obj_square(cx*nx+(i-sqnobjs/2)*2.5*cr*nx,cy*ny+(j-sqnobjs/2)*2.5*cr*ny,cr*nx,grav);
                            objtext="square";
                            break;
                    case 4: os=new obj_flapper(cx*nx,cy*ny,0.1*cr*nx,cr*nx,0.06*cr*nx,0.6*cr*nx,0.,2.1,80000.);
                            objtext="flapper";
                            break;
                    case 5: os=new obj_smooth_rotor(cx*nx,cy*ny,cr*nx,3.5*cr*nx,cr*nx,0.5*cr*nx,M_PI,M_PI,2*M_PI/niters);
                            objtext="smooth rotor";
                            break;
                    case 6: os=new obj_seven_point(cx*nx,cy*ny,70.,10.,0.5*M_PI,2*M_PI/50000.);
                            objtext="7-point rotor";
                            break;
                    case 7: os=new obj_ellipse(cx*nx+(i-sqnobjs/2)*2.5*cr*nx,cy*ny+(j-sqnobjs/2)*2.5*cr*ny,cr*nx,1.,1.5,grav);
                            objtext="ellipse";
                            break;
                }
                objs.push_back(os);
        ...
        printf("shear wave timestep limit               dt_I:          %g\n"
               ...
               "x position of solid center              cx:            %g\n"
               "y position of solid center              cy:            %g\n"
               "length scale of solid object            cr:            %g\n"
               "==========================================================================\n",
               dt1,objtext.c_str(),rho_s,G,cx,cy,cr);
    }
